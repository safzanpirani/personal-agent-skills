...
2026 and has three defects this workflow depends on: `start --run` never exits
(hangs the agent), `console.error(...)` is captured but reported as zero errors,
and `--output` strands the session so the recording finalises at 0 bytes.
Verify with `proofshot doctor`.
